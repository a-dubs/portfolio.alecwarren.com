...
function custom_sub_gallery() {
    var slidewidth = $("#custom_sub_gallery").width()
    var animationduration =600;
    // Comment the line below out if you do not want the automatic slide changing 
    var currentslide = 1;
    var custom_sub_gallery_timer = setInterval(goToSlide, 4000);
    ...
    $("#custom_sub_gallery #bar_island_ul").get(0).appendChild(tempbar);
    var bar_opacity = $("#custom_sub_gallery .bar").css("opacity");
    $("#custom_sub_gallery .bar").remove()
    ...
    function goToSlide(slideNumber = currentslide + 1) {
     if(slideNumber>numofslides){
        ...
        if (!$(jQuery(slides[oldslide - 1])).is(':animated')) {
            clearInterval(custom_sub_gallery_timer);
            custom_sub_gallery_timer = setInterval(goToSlide, 5000);
    ...
    var oldslide = 1;
    var slides = document.getElementById("custom_sub_gallery").querySelectorAll(".slide");

    var numofslides = $('#custom_sub_gallery #carousel').children().length;
    var min = 0;

    var gallery_btn_hovered = false;

    //$("#custom_sub_gallery .slide").css("width", slidewidth);
    ...
    $("#custom_sub_gallery .gallery-btn#left").click(function() {
        if (!$(jQuery(slides[oldslide - 1])).is(':animated')) {
            if (currentslide == 1) {
    ...
    $("#custom_sub_gallery .gallery-btn#right").click(function() {
        if (!$(jQuery(slides[oldslide - 1])).is(':animated')) {
    ...
    for (i = 0; i < numofslides; i++) {
        ...
        $("#custom_sub_gallery #bar_island_ul").get(0).appendChild(newbar);
    }

    // create inner bars
    var bars = document.getElementById("custom_sub_gallery").querySelectorAll(".bar")
    ...
    $("#custom_sub_gallery .bar").hover(function() {
        $(this).stop().animate({
            "opacity": 1
        }, 150)
    }, function() {
    ...
    var gallery_btn_opacity = $("#custom_sub_gallery .gallery-btn").css("opacity");

    // animate slider buttons when hovered
    $("#custom_sub_gallery .gallery-btn").hover(function() {
    ...
    $("#custom_sub_gallery .bar").click(function() {
        if (!$(jQuery(slides[oldslide - 1])).is(':animated')) {
            if (!$(this).hasClass("selected")) {
    ...
    function resizeGallery() {
        slidewidth = document.getElementById("custom_sub_gallery").offsetWidth;
        farthest = ((slides - 1) * slidewidth);
        clearInterval(custom_sub_gallery_timer);
        custom_sub_gallery_timer = setInterval(goToSlide, 5000);
        slideduration = slidewidth * .75;
        $("#custom_sub_gallery .slide").css("width", slidewidth);
        $("#custom_sub_gallery #carousel").css("width", slidewidth * slides);
    ...
    $("#custom_sub_gallery").resize(resizeGallery);
    document.getElementById("custom_sub_gallery").addEventListener("resize", resizeGallery);
}
custom_sub_gallery();
