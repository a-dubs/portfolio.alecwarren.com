...
def make_project_card(project_info : dict[str, str]) -> str:
    ...
    '_gallery" class="gallery col-24 col-sm-20 col-md-18 col-lg-17">\n' + \
    '                <ul id="carousel">\n' + \
    ...
    '            </div>\n' + \
    '\n' + \
    '            <div class="summary col-22 col-sm-19 col-md-18 col-lg-17">\n'+\
    '                <h3>Summary</h3>\n' + \
                        pi["summary"] + '\n' + \
    '            </div>\n' + \
                 make_c2a_button(pi) + \
    ...
    '    </div>\n' + \
    '</div>\n' 
    ) if "image gallery" in pi else (  ################ No image gallery: #################
    '<div id="' + pi["name"] +'" class="container project">\n' + \
    '\n' + \
        make_project_metadata(pi) + \
    '    <div class="collapse-area">\n' + \
...
def add_gallery_javascript(project_name):
    ...
    with open("github automation/GalleryName-gallery-script.js", "r") as template_js_file:
...
def download_image_from_github(download_url : str, local_file_path : str):
    try: 
        
        if not os.path.exists(dir:=local_file_path[:local_file_path.rfind("/")]):
        ...
        print("download failed:",download_url,"\n->",local_file_path)
        return None
