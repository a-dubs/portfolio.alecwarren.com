...
function smart_deadbolt_gallery() {
    var slidewidth = $("#smart_deadbolt_gallery").width()
    ...
    $("#smart_deadbolt_gallery #bar_island_ul").get(0).appendChild(tempbar);
    var bar_opacity = $("#smart_deadbolt_gallery .bar").css("opacity");
    $("#smart_deadbolt_gallery .bar").remove()
    ...
    var oldslide = 1;
    var slides = document.getElementById("smart_deadbolt_gallery").querySelectorAll(".slide");

    var numofslides = $('#smart_deadbolt_gallery #carousel').children().length;
    var min = 0;

    var gallery_btn_hovered = false;

    //$("#smart_deadbolt_gallery .slide").css("width", slidewidth);
    ...
    for (i = 0; i < numofslides; i++) {
        ...
        $("#smart_deadbolt_gallery #bar_island_ul").get(0).appendChild(newbar);
    }

    // create inner bars
    var bars = document.getElementById("smart_deadbolt_gallery").querySelectorAll(".bar")
    ...
    $("#smart_deadbolt_gallery .bar").hover(function() {
        $(this).stop().animate({
            "opacity": 1
        }, 150)
    }, function() {
    ...
    var gallery_btn_opacity = $("#smart_deadbolt_gallery .gallery-btn").css("opacity");

    // animate slider buttons when hovered
    $("#smart_deadbolt_gallery .gallery-btn").hover(function() {
    ...
    function resizeGallery() {
        slidewidth = document.getElementById("smart_deadbolt_gallery").offsetWidth;
        farthest = ((slides - 1) * slidewidth);
        clearInterval(smart_deadbolt_gallery_timer);
        smart_deadbolt_gallery_timer = setInterval(goToSlide, 5000);
        slideduration = slidewidth * .75;
        $("#smart_deadbolt_gallery .slide").css("width", slidewidth);
        $("#smart_deadbolt_gallery #carousel").css("width", slidewidth * slides);
    ...
    $("#smart_deadbolt_gallery").resize(resizeGallery);
    document.getElementById("smart_deadbolt_gallery").addEventListener("resize", resizeGallery);
}
smart_deadbolt_gallery();
