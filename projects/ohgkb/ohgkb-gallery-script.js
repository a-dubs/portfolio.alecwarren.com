...
function ohgkb_gallery() {
    var slidewidth = $("#ohgkb_gallery").width()
    var animationduration =600;
    // Comment the line below out if you do not want the automatic slide changing 
    var currentslide = 1;
    var ohgkb_gallery_hidden = $("#ohgkb_gallery").is(":visible");
    var ohgkb_gallery_timer = setInterval(goToSlide, 5000);
    ...
    $("#ohgkb_gallery #bar_island_ul").get(0).appendChild(tempbar);
    var bar_opacity = $("#ohgkb_gallery .bar").css("opacity");
    $("#ohgkb_gallery .bar").remove()
    ...
    function goToSlide(slideNumber = currentslide + 1) {
        if ($("#ohgkb_gallery").is(":visible")) {
            
            if (ohgkb_gallery_hidden)
            {
                clearInterval(ohgkb_gallery_timer);
                ohgkb_gallery_timer = setInterval(goToSlide, 5000);
                ohgkb_gallery_hidden = false;
            }    
            else {
            ...
            ohgkb_gallery_hidden = true;
            clearInterval(ohgkb_gallery_timer);
            ohgkb_gallery_timer = setInterval(goToSlide, 100);
        }

    }

    var oldslide = 1;
    var slides = document.getElementById("ohgkb_gallery").querySelectorAll(".slide");

    var numofslides = $('#ohgkb_gallery #carousel').children().length;
    var min = 0;

    var gallery_btn_hovered = false;

    //$("#ohgkb_gallery .slide").css("width", slidewidth);
    ...
    $("#ohgkb_gallery .gallery-btn#left").click(function () {
        ohgkb_gallery_hidden = false;
        if (!$(jQuery(slides[oldslide - 1])).is(':animated')) {
            if (currentslide == 1) {
    ...
    $("#ohgkb_gallery .gallery-btn#right").click(function () {
        ohgkb_gallery_hidden = false;
        if (!$(jQuery(slides[oldslide - 1])).is(':animated')) {
    ...
    for (i = 0; i < numofslides; i++) {
        ...
        $("#ohgkb_gallery #bar_island_ul").get(0).appendChild(newbar);
    }

    // create inner bars
    var bars = document.getElementById("ohgkb_gallery").querySelectorAll(".bar")
    ...
    $("#ohgkb_gallery .bar").hover(function() {
        $(this).stop().animate({
            "opacity": 1
        }, 150)
    }, function() {
    ...
    var gallery_btn_opacity = $("#ohgkb_gallery .gallery-btn").css("opacity");

    // animate slider buttons when hovered
    $("#ohgkb_gallery .gallery-btn").hover(function() {
    ...
    $("#ohgkb_gallery .bar").click(function () {
        ohgkb_gallery_hidden = false;
        if (!$(jQuery(slides[oldslide - 1])).is(':animated')) {
            if (!$(this).hasClass("selected")) {
    ...
    function resizeGallery() {
        slidewidth = document.getElementById("ohgkb_gallery").offsetWidth;
        farthest = ((slides - 1) * slidewidth);
        clearInterval(ohgkb_gallery_timer);
        ohgkb_gallery_timer = setInterval(goToSlide, 5000);
        slideduration = slidewidth * .75;
        $("#ohgkb_gallery .slide").css("width", slidewidth);
        $("#ohgkb_gallery #carousel").css("width", slidewidth * slides);
    ...
    $("#ohgkb_gallery").resize(resizeGallery);
    document.getElementById("ohgkb_gallery").addEventListener("resize", resizeGallery);
}
ohgkb_gallery();
