...
function smart_bedroom_leds_gallery() {
    var slidewidth = $("#smart_bedroom_leds_gallery").width()
    var animationduration =600;
    // Comment the line below out if you do not want the automatic slide changing 
    var currentslide = 1;
    var smart_bedroom_leds_gallery_hidden = $("#smart_bedroom_leds_gallery").is(":visible");
    var smart_bedroom_leds_gallery_timer = setInterval(goToSlide, 5000);
    ...
    $("#smart_bedroom_leds_gallery #bar_island_ul").get(0).appendChild(tempbar);
    var bar_opacity = $("#smart_bedroom_leds_gallery .bar").css("opacity");
    $("#smart_bedroom_leds_gallery .bar").remove()
    ...
    function goToSlide(slideNumber = currentslide + 1) {
        if ($("#smart_bedroom_leds_gallery").is(":visible")) {
            
            if (smart_bedroom_leds_gallery_hidden)
            {
                clearInterval(smart_bedroom_leds_gallery_timer);
                smart_bedroom_leds_gallery_timer = setInterval(goToSlide, 5000);
                smart_bedroom_leds_gallery_hidden = false;
            }    
            else {
            ...
            smart_bedroom_leds_gallery_hidden = true;
            clearInterval(smart_bedroom_leds_gallery_timer);
            smart_bedroom_leds_gallery_timer = setInterval(goToSlide, 100);
        }

    }

    var oldslide = 1;
    var slides = document.getElementById("smart_bedroom_leds_gallery").querySelectorAll(".slide");

    var numofslides = $('#smart_bedroom_leds_gallery #carousel').children().length;
    var min = 0;

    var gallery_btn_hovered = false;

    //$("#smart_bedroom_leds_gallery .slide").css("width", slidewidth);
    ...
    $("#smart_bedroom_leds_gallery .gallery-btn#left").click(function () {
        smart_bedroom_leds_gallery_hidden = false;
        if (!$(jQuery(slides[oldslide - 1])).is(':animated')) {
            if (currentslide == 1) {
    ...
    $("#smart_bedroom_leds_gallery .gallery-btn#right").click(function () {
        smart_bedroom_leds_gallery_hidden = false;
        if (!$(jQuery(slides[oldslide - 1])).is(':animated')) {
    ...
    for (i = 0; i < numofslides; i++) {
        ...
        $("#smart_bedroom_leds_gallery #bar_island_ul").get(0).appendChild(newbar);
    }

    // create inner bars
    var bars = document.getElementById("smart_bedroom_leds_gallery").querySelectorAll(".bar")
    ...
    $("#smart_bedroom_leds_gallery .bar").hover(function() {
        $(this).stop().animate({
            "opacity": 1
        }, 150)
    }, function() {
    ...
    var gallery_btn_opacity = $("#smart_bedroom_leds_gallery .gallery-btn").css("opacity");

    // animate slider buttons when hovered
    $("#smart_bedroom_leds_gallery .gallery-btn").hover(function() {
    ...
    $("#smart_bedroom_leds_gallery .bar").click(function () {
        smart_bedroom_leds_gallery_hidden = false;
        if (!$(jQuery(slides[oldslide - 1])).is(':animated')) {
            if (!$(this).hasClass("selected")) {
    ...
    function resizeGallery() {
        slidewidth = document.getElementById("smart_bedroom_leds_gallery").offsetWidth;
        farthest = ((slides - 1) * slidewidth);
        clearInterval(smart_bedroom_leds_gallery_timer);
        smart_bedroom_leds_gallery_timer = setInterval(goToSlide, 5000);
        slideduration = slidewidth * .75;
        $("#smart_bedroom_leds_gallery .slide").css("width", slidewidth);
        $("#smart_bedroom_leds_gallery #carousel").css("width", slidewidth * slides);
    ...
    $("#smart_bedroom_leds_gallery").resize(resizeGallery);
    document.getElementById("smart_bedroom_leds_gallery").addEventListener("resize", resizeGallery);
}
smart_bedroom_leds_gallery();
